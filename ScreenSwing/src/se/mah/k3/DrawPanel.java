...
public class DrawPanel extends JPanel implements Runnable{
	...
	Font font = new Font("Roboto-Medium", Font.BOLD, 20);
	...
	Color wordBackgroundColor = (hexToRgb("#009688"));
	...
	public DrawPanel() {
		...
		// If the word is active - for testing purposes
		w.active = true;
	...
	public void paint(Graphics g) {
		...
		g2.drawImage(backgroundImage, 0, 0, WIDTH + 1, HEIGHT + 1, this);
		...
		for(int i=0; i<particles.size();i++){ // run all particles
			...
			//for(Word w:words){
			if(w.active)particles.get(i).collisionCircle(w.xPos,w.yPos);
			//}
		}

		//for(int i=0; i<words.size();i++){
		if(w.active){
			g2.setColor(wordBackgroundColor);
			g2.fillRect((int)(w.xPos-(w.width*0.25)), (int)(w.yPos-(w.height*0.5)), w.width, w.height);
			g2.setColor(Color.white);
			g2.drawString(w.getText(), w.xPos, w.yPos);
		}
		//}
	}
